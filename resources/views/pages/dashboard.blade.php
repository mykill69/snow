...
@section('body')
            ...
            $('#tickets-range').daterangepicker({
                ...
                function(start, end) {
                    $('#tickets-range span').html(start.format('MMMM D, YYYY') + ' - ' + end.format(
                        'MMMM D, YYYY'));
                    // Optional: Call function here to filter data based on selected date
                }
            );
        });
    </script> --}}
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            /* -------------------------------------------------------------------
             *  Helper – pick the needle colour for any 1‑to‑5 score
             * ------------------------------------------------------------------- */
            const getGaugeColor = v =>
                v >= 4 ? '#FFB140' // orange‑gold
                :
                v >= 3 ? '#ffc107' // yellow‑orange
                :
                '#dc3545'; // red
            ...
            /* -------------------------------------------------------------------
             *  GAUGE 1 – Overall average
             * ------------------------------------------------------------------- */
            const overallAvg = @json($overallAvg); // numeric 0‑5
            new Chart(
                document.getElementById('gaugeChart').getContext('2d'), {
                    ...
                    data: {
                        datasets: [{
                            ...
                            backgroundColor: [getGaugeColor(overallAvg), '#4E6766'],
                            borderWidth: 0
                        }]
                    }
                }
            ...
            /* -------------------------------------------------------------------
             *  GAUGE 2 – Current month average
             * ------------------------------------------------------------------- */
            const currentAvg = @json($currentAvg);
            new Chart(
                document.getElementById('gaugeChart2').getContext('2d'), {
                    ...gaugeBaseOptions,
                    data: {
                        datasets: [{
                            data: [currentAvg, 5 - currentAvg],
                            backgroundColor: [getGaugeColor(currentAvg), '#4E6766'],
                            borderWidth: 0
                        }]
                    }
                }
        ...
        const centerTextPlugin = {
            ...
            beforeDraw(chart) {
                ...
                const textX = Math.round((width - ctx.measureText(centerText).width) / 2) - 41;
    ...
    <script>
        ...
        for (let i = ticketForecastDates.length - 1; i >= 0 && addedHistorical < 5; i--) {
            ...
            if (dayOfWeek !== 0 && dayOfWeek !== 6) { // skip weekends
                historicalLabels.unshift(d.toLocaleDateString('en-US', {
                    month: 'short',
                    day: 'numeric',
                    year: 'numeric'
                }));
        ...
        // Highlight the current date with different color and bigger point
        const todayStr = new Date().toLocaleDateString('en-US', {
            month: 'short',
            day: 'numeric',
            year: 'numeric'
        });
        const pointColors = ticketForecastLabels.map(label => (label === todayStr ? '#FF0000' : 'black')); // red for today
        const pointSizes = ticketForecastLabels.map(label => (label === todayStr ? 6 : 4)); // bigger for today
